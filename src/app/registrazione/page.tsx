...
export default function RegistrazionePage() {
  ...
  const validate = (state: FormState) => {
    ...
    if (!state.dealerName.trim()) {
      newErrors.dealerName = "Inserisci il nome della concessionaria.";
    }

    if (!state.companyName.trim()) {
      newErrors.companyName = "Inserisci la ragione sociale.";
    }
    ...
    if (!state.contactName.trim()) {
      newErrors.contactName = "Inserisci il nome del referente.";
    }
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const { error: insertError } = await supabase.from("dealers").insert(dealerPayload);

    if (insertError) {
      setServerMessage(insertError.message);
      setIsSubmitting(false);
      return;
    }
            ...
            <div className="mx-auto max-w-2xl">
              <div className="mb-8">
                ...
                <p className="mt-3 text-sm leading-6 text-slate-600 sm:text-base">
                  Compila le informazioni per iniziare. Tutti i campi sono obbligatori.
                </p>
              </div>
              ...
              <form className="space-y-6" onSubmit={handleSubmit} noValidate>
                <div className="grid gap-4 sm:grid-cols-2">
                  <div>
                    <label htmlFor="dealerName" className="mb-2 block text-sm font-medium text-slate-700">
                      Company Name
                    </label>
                    <input
                      id="dealerName"
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <div>
                    <label htmlFor="vatNumber" className="mb-2 block text-sm font-medium text-slate-700">
                      VAT Number
                    </label>
                    <input
                      id="vatNumber"
                  ...
                  <div>
                    <label htmlFor="taxCode" className="mb-2 block text-sm font-medium text-slate-700">
                      Tax Code
                    </label>
                    <input
                      id="taxCode"
